...
class Application
  ...
  def build_files
    ...
    sorted_patterns.each do |pattern|
      File.open("source/patterns/#{pattern.permalink}.markdown", "w") do |file|
        file.write("---title:#{pattern.name}\n---\n#{ReverseMarkdown.convert(pattern.notes_html).inspect}")
      end
    end

    # make pattern index
    # File.open("source/patterns/index.html", "w") do |file|
      # stuff
    # end
  end

  private def pattern_index_format(pattern)
    # todo
    # format pattern
  end
end
